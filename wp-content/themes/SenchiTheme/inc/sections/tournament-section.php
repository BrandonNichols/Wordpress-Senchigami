        ...
        <div class="container">
            <div class="section-header border-bottom">
                <div class="section-title">
                    <h2><?php echo esc_html__('Tournaments', 'pixiefreak'); ?></h2>
                </div>
                ...
                <ul>
					<?php $activeStatus = ''; foreach ($statuses = \PixieFreakPanel\Model\Tournament::STATUSES as $key => $status): ?>
						...
						?>
                        <li class="<?php echo esc_attr($activeStatus == $status ? 'active' : ''); ?>">
                            <a href="#<?php echo esc_attr(strtolower($status) .'-tournament'); ?>" data-toggle="tab">
								<?php if ($status == 'Upcoming'): ?>
									<?php echo esc_html__('Upcoming', 'pixiefreak'); ?>
								<?php elseif($status == 'Finished'): ?>
									<?php echo esc_html__('Finished', 'pixiefreak'); ?>
								<?php else: ?>
									<?php echo esc_html__('Started', 'pixiefreak'); ?>
								<?php endif; ?>
                            </a>
                        </li>
					<?php endforeach; ?>
            ...
            <div class="tab-content">
				<?php foreach ($statuses as $key => $status):  ?>
                    <ul id="<?php echo esc_attr(strtolower($status) .'-tournament'); ?>"
                        class="tournament-list<?php echo esc_attr($activeStatus == $status ? ' active' : ''); ?>">
                                ...
                                <div class="tournament-footer">
                                    <div class="col">
                                        ...
                                        <div class="col-item">
                                            ...
                                            <p><?php echo esc_html($tournament->team_number); ?> <?php echo esc_html__('Teams', 'pixiefreak'); ?></p>
                                        </div>
                                    ...
                                    <div class="col align-right">
                                        <a href="<?php echo pixiefreak_route('tournament', $tournament->slug); ?>" class="btn-default">
											<?php echo esc_html__('More details', 'pixiefreak'); ?> <i class="fas fa-arrow-right"></i>
                                        </a>
                                    </div>
                                </div>
